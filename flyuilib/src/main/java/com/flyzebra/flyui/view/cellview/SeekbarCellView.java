    ...
    public void upData(CellBean cellBean) {
        ...
        if (mCellBean.subCells != null && mCellBean.subCells.size() >= 5) {
            ...
            if (startCell.celltype == CellType.TYPE_TEXT) {
                ...
                LayoutParams lp = new LayoutParams(startCell.width, startCell.height);
                lp.setMarginStart(0);
                addView(startTV, lp);
            }
            ...
            if (endCell.celltype == CellType.TYPE_TEXT) {
                ...
                LayoutParams lp = new LayoutParams(endCell.width, endCell.height);
                lp.setMarginStart(mCellBean.width - endCell.width);
                addView(endTV, lp);
            }
        ...
        if (key == ActionKey.MEDIA_TIME) {
            if (obj instanceof long[]) {
                ...
                if (intarr.length == 2) {
                    ...
                    if (startTV != null) {
                        startTV.setText("00" + intarr[0]);
                    }
                    if (endTV != null) {
                        endTV.setText("11" + intarr[1]);
                    }
                }
            }
        }
